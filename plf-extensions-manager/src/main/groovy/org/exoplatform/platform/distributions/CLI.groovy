...
class CLI {
  ...
  static boolean initialize(String[] args) {
    ...
    cli.with {
      ...
      s longOpt: 'snapshots', 'List also all add-ons SNAPSHOTs'
    }
    ...
    if (options.l) {
      ...
      Settings.instance.extensionId = options.i
      if (options.f) {
      ...
      Settings.instance.extensionId = options.u
    } else {
      Logging.displayMsgError("Invalid command line parameter(s)")
      cli.usage()
      return false
